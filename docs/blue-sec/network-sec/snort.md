...
|`-v`| This will start snort in the **verbose mode (-v)**|`sudo snort -v`|
...
|`-Q --daq afpacket` `-i`| This will start snort with an activated **Data Acquisition (DAQ)** module and use the `afpacket` module to use snort as an IPS on the given interface **eth0:eth1**. The DAQ is needed for enabling snort to drop packages.|`sudo snort -c /etc/snort/snort.conf -q -Q --daq afpacket -i eth0:eth1 -A console`|
