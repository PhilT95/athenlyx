...
The installation of [SafeLine](https://github.com/chaitin/SafeLine) is rather easy, since the developers provide a simple docker setup. A few dependencies and requirements need to be checked and verified before you can begin with the installation and configuration.
...
You can access the certificate management via the **Settings** menu on the left. Search the **SSL Cert** tab and click the **Add Cert** button.

![Image](images/safeline_certificate-add.png)

You can now see the 2 options mentioned earlier. Either you upload the *Cert* and *Key* file from an SSL certificate you already have or you click **Get free cert**. Here you can enter one or more domains you want to generate an SSL Certificate for. Provide an email address for your certificate and click **Submit**.

!!! Warning
    Please make sure your SafeLine instance is reachable using these Domains from the Internet.
...
Moving forward to secure your application, you need to go the **Applications** tab on the left.

![Image](images/safeline_applications.png)

Click the **Add application** button on the top right of this tab. Here you are asked to fill in the following information:
...
- **Forwarding**: select how SafeLine forwards the traffic. It can act as one of the following:
    ...
    - **Reverse Proxy**: if chosen, SafeLine uses *nginx* to proxy the web traffic to the given web server. The configuration of nginx is automatically created by SafeLine. You need to provide an **Upstream** target, which is normally the internal IP address or DNS name of your web server.
...
!!! note "SSL tick box"
    The SSL tick box indicates which of the configured ports is using SSL. You can, for example, add a custom port for an web application that doesn't use port 443 for HTTP/S easy this way. If you don't tick the SSL box you get errors accessing the web application using this port.

Once you created your application, it appears in the applications menu. You can click **Details** to see the configuration. This details pane provides the security and log settings. SafeLine provides 4 different security measures:
...
- **Auth**: SafeLine offers the possibility to put websites behind an additional *Authentication Challenge*. This feature requires additional authentication methods like SafeLine-local users or connected SSO/Third-Party Logins.
- **Attacks**: this feature, when enabled, stops web-based attacks before they reach the web server. It is based on[ModSecurity](https://modsecurity.org/) and can be configured to different intensity degrees.
...
To enable the **Access** and **Error** logs, click one of the logs, and hit the **Enable** checkbox.
